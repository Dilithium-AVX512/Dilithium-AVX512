...
#elif L == 4
  poly_uniform_gamma1_8x(&z.vec[0], &z.vec[1], &z.vec[2], &z.vec[3], &tmp, &tmp1, &tmp2, &tmp3,
                         rhoprime, nonce, nonce + 1, nonce + 2, nonce + 3, 0, 1, 2, 3);
  nonce += 4;
#elif L == 5
  poly_uniform_gamma1_8x(&z.vec[0], &z.vec[1], &z.vec[2], &z.vec[3], &z.vec[4], &tmp, &tmp1, &tmp2,
                         rhoprime, nonce, nonce + 1, nonce + 2, nonce + 3, nonce + 4, 0, 1, 2);
  nonce += 5;
#elif L == 7
  poly_uniform_gamma1_8x(&z.vec[0], &z.vec[1], &z.vec[2], &z.vec[3], &z.vec[4], &z.vec[5], &z.vec[6], &tmp,
                         rhoprime, nonce, nonce + 1, nonce + 2, nonce + 3, nonce + 4, nonce + 5, nonce + 6, 0);
  nonce += 7;
#else
#error
#endif
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen, const uint8_t *m, size_t mlen, const uint8_t *pk)
{
  ...
  ALIGNED_UINT8(K * POLYW1_PACKEDBYTES + 14)
  buf;
  uint8_t mu[CRHBYTES];
